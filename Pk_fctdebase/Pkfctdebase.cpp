...
vector<vector<int>> sort_partitions_geometric(vector<vector<int>> partitions, int K) {
    ...
    sort(edge1.begin(), edge1.end(), [](const vector<int> &a, const vector<int> &b) { return a[0] > b[0]; });

    // Trier arete 2 (j=0): i croissant
    sort(edge2.begin(), edge2.end(), [](const vector<int> &a, const vector<int> &b) { return a[0] < b[0]; });

    // Trier arete 3 (i=0): j decroissant
    sort(edge3.begin(), edge3.end(), [](const vector<int> &a, const vector<int> &b) { return a[1] > b[1]; });
...
void BasisFctPK(int K, vector<vector<int>> &coef, vector<vector<int>> &shift, vector<int> &ff, vector<int> &il,
                vector<int> &jl, vector<int> &kl) {
    // Silverster formula ~1968
