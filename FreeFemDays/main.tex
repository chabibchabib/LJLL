...
\uncover<2>{
  \begin{tikzpicture}[remember picture, overlay]
      ...
      \node at (current page.north west) [xshift=8.5cm, yshift=-5.8cm] {
      \begin{tikzpicture}
          \draw [color=blue,decorate,decoration={amplitude=10mm}](0.4,0) rectangle (6.8,0.5);
        %\node at (8,0.25) {\textcolor{blue}{}};
        \end{tikzpicture}
      };
  \end{tikzpicture}
}
...
\begin{tabular}{ c c c }
 ...
 Dimension of the function space & \texttt{1} & \texttt{1}  \\
 Array containing topological information & \texttt{Data} & \texttt{Data}  \\  
 Number of subdivisions for plotting  & \texttt{4} & $k$  \\  
 Number of component sub-finite elements & \texttt{1} & \texttt{1}  \\  
 nombre de dof + dof permutation non sym sur aretes & \texttt{15+6} & cell9  \\  
 Number of interpolation points & \texttt{15} & $\displaystyle\frac{(k+1)(k+2)}{2}$  \\  
 dof dont on a la connaissance avant  & \texttt{0} & \texttt{0}  \\  
