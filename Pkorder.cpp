...
void dump(int l, int *t, ostream &cc = cout) {
    cc.precision(20);
    cc << " { ";
    for (int i = 0; i < l; i++)
        cc << t[i] << " " << (i < l - 1 ? ',' : '}');
...
void dump2(int l, Tab lx, Tab ly, int k, ostream &cc = cout) {
    cc << " { ";
    for (int i = 0; i < l; i++)
...
void dump(int l, int ll, TTab t, ostream &cc = cout) {
    cc << " { \n";
    for (int i = 0; i < l; i++) {
        cc << "\t\t";
        dump(ll, t[i], cc);
